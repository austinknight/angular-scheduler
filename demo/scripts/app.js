            ...
            _.each(schedule, function(bucket){
              ...
              if (bucket.date == currentDate) {
                ...
                _.each(bucket, function(item, index){
                  if (item.start < daysList[0].id && item.end > daysList[0].id) {
                    item.start = daysList[0].id;
                    item.duration = moment(daysList[0].id).diff(item.end, 'days') * -1;
                  }
                  ...
                  if (item.start >= daysList[0].id && item.start <= daysList[daysList.length - 1].id && item.end > daysList[daysList.length - 1].id) {
                    item.end = daysList[daysList.length - 1].id;
